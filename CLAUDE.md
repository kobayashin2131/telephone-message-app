...
**ログイン機能の要件（オーナー確定事項）**:
- ログインIDは既存の `email` 列をそのまま使う
- パスワードは **PINレベルの短い数字でOK**（クローズドな社内ツールのため。ただしハッシュ化はする — CloudflareWorkersはbcrypt非対応なのでWeb CryptoのPBKDF2等を使う）
- パスワードは **本人が変更 / 管理者がリセット** できるようにする
- 今の「ヘッダーのアカウント切替ドロップダウン」（誰でも他人になりすませる作り）は本物のログイン/ログアウトに置き換える
- 各API呼び出しの `user_id`/`created_by` は現状クライアントが自己申告した値をそのまま信用しており誰でも偽装できる。ログイン後はサーバー側でトークンから本人を特定する形に直す

**将来の収益化モデル（オーナー確定事項）**: 1社＝1テナント（完全にクローズドな環境）を売る形。購入した会社のオーナー（管理者）が最初のアカウントを作り、そこから社員を追加していく、標準的なB2B SaaSの広がり方。
...
### Phase 2: ログイン機能
- `users.password`（現状 平文で `password123` 等が入っている）→ `password_hash` に置き換え
- セッション管理方式を決める（`sessions` テーブル方式 or 署名付きトークン方式）
- 新エンドポイント: `POST /api/auth/login`（organization特定＋email＋password→token）, `POST /api/auth/change-password`（本人用）, `POST /api/auth/reset-password`（管理者専用、PINリセット）
- フロント: ログイン画面を新設。ヘッダーのアカウント切替ドロップダウンを廃止しログアウトボタンに置き換え
- 全fetchにトークンを付与し、バックエンド側で `user_id` をトークンから解決する形に変更
