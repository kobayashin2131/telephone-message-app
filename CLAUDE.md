...
**ログイン機能の要件（2026-08-19 最終決定・PIN案から変更）**:
- **メール＋パスワード/PINではなく「Googleでログイン」（OAuth）にする。** オーナーの会社は既にGoogle Chat（無料版）を使っており、全社員（想定60名規模）が既にGoogleアカウントを持っている。Google Workspaceの有料化は不要（Google Sign-Inの利用自体は無料）。
- 自前でのパスワード保存・ハッシュ化・変更・管理者リセットの実装は **不要になる**（Googleが認証を肩代わりするため）。上記の「PINレベルのパスワード」案・`password_hash`列・reset-passwordエンドポイントは全て破棄してよい。
- ログインIDは引き続き既存の `email` 列と一致させる（Googleアカウントの検証済みメールアドレスをそのまま使う）。
- 実装イメージ: Google OAuth（Authorization Code Flow、またはGoogle Identity ServicesのSign In With Googleボタン）→ Googleから検証済みメールアドレスを受け取る → `users.email` と突き合わせて本人特定 → セッショントークン発行。
- 今の「ヘッダーのアカウント切替ドロップダウン」（誰でも他人になりすませる作り）はGoogleログイン/ログアウトに置き換える。
- 各API呼び出しの `user_id`/`created_by` は現状クライアントが自己申告した値をそのまま信用しており誰でも偽装できる。ログイン後はサーバー側でセッショントークンから本人を特定する形に直す。
- **Googleアカウントを持たない人向けの「ID＋PIN」ログインは今回は作らない。** ハイブリッド構成（Google＋ID/PIN両対応）も技術的には可能とオーナーに説明済みだが、実装量がほぼ倍になる（結局パスワード保存・ハッシュ化・リセット機能一式が必要になるため）ので、オーナー了承の上でスコープ外にした。**将来、実際にGoogleアカウントを持たない利用者が出てきたら後から追加する**という前提。設計上、その追加を妨げるような作り（Googleのみを前提にした密結合など）にしないよう注意。
