...
## 📋 2026-08-21：電話メモ専用通知欄の方針検討（未実装・次回着手予定）

**背景**: 電話メモがチャットのメッセージ一覧に混ざって表示される今の仕様について、オーナーから「電話連絡を埋もれさせない」というこの製品そもそもの存在意義と矛盾しかねない、という指摘。壁打ちの結果、以下の方針で合意（**まだコードは書いていない、次回このセッションか別セッションで実装**）:

- チャットのサイドバーに「📞受電メモ」専用の一覧欄（自分・自分のグループ宛ての受電メモを新着順に表示）を新設し、これを**唯一の「電話メモの入り口」**とする
- チャット内メッセージとして流れる受電メモカード（`CallMemoCard`、現状の実装）は、会話の文脈がわかる軽量な参照として残す（削除はしない）
- 新着があるときだけ**目立つバナー的な通知**を表示し、それをタップ/クリックすると上記の専用欄が開く、という2段階の見せ方にする（「ボタンだけ」だと今気づいた人にしか効かず、後から追いつきたい人向けの一覧としての機能が弱くなるため、常設の一覧＋新着時のバナーの組み合わせで合意）
- 入り口を1箇所に一本化することで、「チャット内カード」「専用欄」「CallSyncアプリ本体」の3箇所に情報が散らばって「どこが正解かわからない」状態になることを避ける

**次にこの作業をする人へ**: 実装時は、既存の`callMemosCount`（ヘッダーの「N件 未対応」バッジ、`AppHeader.jsx`）との役割分担を整理すること。バッジは残すのか、専用欄のバナーに統合するのか、要検討。
